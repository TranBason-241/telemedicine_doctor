...
const MultipleAutocompleteActive = <T extends Record<string, string>>(
        ...
        async (query: string, limit: number, searchValue: string) => {
            ...
            try {
                const response = await axios.get<IPagingSupport<T>>(
                    `${query}?${searchField}=${searchValue}&is-active=true&page-offset=1&limit=${limit}`
                );
...
export default MultipleAutocompleteActive;
